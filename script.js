document.addEventListener('DOMContentLoaded', () => {
    ...
    productForm.addEventListener('submit', (e) => {
        ...
        if (isNaN(productPrice) || !['jpg', 'png', 'gif'].includes(productImage.split('.').pop())) {
    ...
    function renderProducts() {
        ...
        products.forEach(product => {
            ...
            productCard.innerHTML = `
                ...
                <input type="checkbox" class="mt-2" data-product-id="${product.id}">
            `;
    ...
    function handleProductSelection() {
        const selectedProductIds = Array.from(document.querySelectorAll('input[type="checkbox"]:checked'))
            .map(cb => parseInt(cb.dataset.productId));
        cartItems = products.filter(product => selectedProductIds.includes(product.id));
        renderCart();
    }
    function renderCart() {
        cart.innerHTML = '';
